...
<%
    ...
    List<Pedido> pedido = (List<Pedido>) request.getAttribute("pedido");
    ...
    <body>
        ...
        <div class="container"> 
            ...
            <form  action="clienteservlet" method="post">
                ...
                <div class="mb-3">
                    <label  class="form-label" style="color: white">CARNET</label>
                    <input type="text" name="carnet"  class="form-control" value="<%= cliente.getCarnet()%>" >
                </div>
                <div class="mb-3">
                    <label  class="form-label" style="color: white">NRo PEDIDO</label>
                    <select name="pedido_id">
                        <% for (Pedido item : pedido) {
                        %>
                        <option value= "<%= item.getId()%>" <%= (cliente.getPedidoId() != null && item.getId() == cliente.getPedidoId().getId()) ? "selected" : ""%>>
                            <%= item.getId()%>
                        </option>
                        <%
                            }
