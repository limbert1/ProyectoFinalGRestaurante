        ...
        <div class="container"> 
         ...
         <form  action="PedidoServlet" method="post">
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="cantidad" value="<%= pedido.getCantidad()%>" class="form-control"  >            
                </div>
                <div class="mb-3">
                    <label  class="form-label" style="color: white">FECHA</label>
                    <input  type="date" name="fecha" value="<%= pedido.getFecha()%>" class="form-control"  >
                </div>
                <div class="mb-3">
                    <label  class="form-label" style="color: white">PLATILLO</label>
                    <select name="platillo_id">
                                <% for (Platillo item : platillos) {%>
                                <option value="<%= item.getId()%>" <%=(pedido.getPlatilloId() != null && item.getId() == pedido.getPlatilloId().getId()) ? "selected" : ""%>>
                                    <%= item.getDescripcion()%>
                                </option>
                                <% }%>
                            </select>
